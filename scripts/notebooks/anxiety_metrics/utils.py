...
def get_dist(method, locations):
    if method == 'wall':
        return get_per_walls(locations)
    elif method == 'corner':
        return get_per_corners(locations)
    else:
        return get_per_center(locations)
...
def calc_final_vals(dists, maxdist, locations, node):
    ...
    return area_time, not_area_time, nan_time, norm_area, norm_not_area
