...
class PredLoader:
    ...
    # get initial and velocity nan
    def get_nan_color_pair(self, color_pair, fiber_redo=False):
        color_df = self.df[self.df['color pair'] == color_pair]
        for index, row in tqdm(color_df.iterrows()):
            locations = load_file(row) if not fiber_redo else load_file_fiber_redo(row)
            if locations is not None:
                self.df.at[index, 'pred'] = True
                initial_nan = get_nan_prec(locations)
                self.df.at[index, 'error'] = initial_nan
        # self.df.to_csv(self.filename, index=False)
